    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="p-4 md:p-6">
        ...
        {!showForm && !showHistory && <>
          ...
          <div className="mt-6 bg-card border-2 border-border p-4"><p className="text-sm text-muted-foreground">PRÁCTICAS REGISTRADAS</p><p className="text-3xl font-bold">{reports.length}</p></div>
        </>}
        ...
        {showHistory && <>
          <div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-3"><button onClick={() => setShowHistory(false)} className="hover:opacity-70"><ArrowLeft className="w-6 h-6" /></button><h2 className="text-2xl font-bold tracking-wide">HISTORIAL DE PRÁCTICAS</h2></div><span className="text-muted-foreground text-sm">{reports.length} registro(s)</span></div>
          <div className="space-y-4">{reports.map(report => { const tab = activeTab[report.id] ?? 'detalle'; return <div key={report.id} className="bg-card border-2 border-border overflow-hidden">
